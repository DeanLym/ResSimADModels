...
## Plot model
p1 = heatmap(reshape(logk, 200, 200), color=cgrad(:jet))
for wl in values(prod_locs)
    i, j, k = wl
    scatter!(p1, i, j, marker=:circle, markersize=100, legend=false)
end
